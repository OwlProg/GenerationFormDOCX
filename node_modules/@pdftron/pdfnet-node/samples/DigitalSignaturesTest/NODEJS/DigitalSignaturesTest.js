...
((exports) => {
  ...
  exports.runDigitalSignatureTest = () => {
    ...
    const main = async () => {
      ...
      try {
        const doc = await PDFNet.PDFDoc.createFromFilePath(input_path + 'tiger.pdf');
        doc.initSecurityHandler();
        ...
        await doc.save(output_path + 'tiger_withApprovalField_output.pdf', PDFNet.SDFDoc.SaveOptions.e_remove_unused);
      } catch (err) {
        console.log(err);
        ret = 1;
      }
      ...
      try {
        await CertifyPDF(input_path + 'tiger_withApprovalField.pdf',
        'PDFTronCertificationSig',
        input_path + 'pdftron.pfx',
        'password',
        input_path + 'pdftron.bmp',
        output_path + 'tiger_withApprovalField_certified_output.pdf');
        await PrintSignaturesInfo(output_path + 'tiger_withApprovalField_certified_output.pdf');
      } catch (err) {
        console.log(err);
        ret = 1;
      }
      ...
      try {
        await SignPDF(input_path + 'tiger_withApprovalField_certified.pdf',
        'PDFTronApprovalSig',
        input_path + 'pdftron.pfx',
        'password',
        input_path + 'signature.jpg',
        output_path + 'tiger_withApprovalField_certified_approved_output.pdf');
        await PrintSignaturesInfo(output_path + 'tiger_withApprovalField_certified_approved_output.pdf');
      } catch (err) {
        console.log(err);
        ret = 1;
      }
      ...
      try {
        await ClearSignature(input_path + 'tiger_withApprovalField_certified_approved.pdf',
        'PDFTronCertificationSig',
        output_path + 'tiger_withApprovalField_certified_approved_certcleared_output.pdf');
        await PrintSignaturesInfo(output_path + 'tiger_withApprovalField_certified_approved_certcleared_output.pdf');
      } catch (err) {
        console.log(err);
        ret = 1;
      }
